...
function StatsContent() {
  const { id } = useParams();
  const params = id;
  const pathName = api.OA_URL + training_chart + `?sow_id=${params}`;
  const trainingDailyUrl =
    api.OA_URL + training_chart_daily + `?sow_id=${params}&type=month`;
  const hourlyName = api.OA_URL + hourly_leads_chart_oa + `?sow_id=${params}`;
  const overallName = api.OA_URL + overall_leads_chart_oa + `?sow_id=${params}`;
  const overallEnroll = api.OA_URL + overall_fse_enroll + `?sow_id=${params}`;
