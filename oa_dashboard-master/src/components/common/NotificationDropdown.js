...
const NotificationDropdown = () => {
  ...
  const arr = [
    {
      name: "hello",
      message: "Profile update feature added.",
    },
  ];
  const [notify, setNotify] = React.useState(1);
  const [notifyArr, setNotifyArr] = React.useState(arr);
  const socketRef = useRef();

  useEffect(() => {
    axios
      .get("https://farming-backend.taskmo.in/test/msg")
      .then((res) => {
        console.log(res.data.chat, "ttt");
        setNotifyArr(res.data.chat);
        setNotify(res.data.chat.length);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  useEffect(() => {
    socketRef.current = io("wss://farming-backend.taskmo.in", {
      transports: ["websocket"],
    });
    console.log(socketRef, "teh");
    socketRef.current.on("message", ({ name, message }) => {
      setNotifyArr([{ name, message }, ...notifyArr]);
      setNotify([{ name, message }, ...notifyArr].length);
    });
        ...
        <DropdownMenu className="dropdown-menu-lg dropdown-menu-end p-0">
          ...
          <TabContent activeTab={activeTab}>
            <TabPane tabId="1" className="py-2 ps-2">
              <SimpleBar style={{ maxHeight: "250px" }} className="pe-2">
              ...
              <div
                ...
                style={{ borderTop: "1px solid #e9ebec", height: "30px" }}
              >
                <button
                  href="#"
                  className="btn btn-link link-danger fw-500"
                  onClick={() => handleNotifyClear()}
                >
                  Clear
                </button>
              </div>
